...
std::vector<BVH::triangleData> RayCaster::findRayIntersection(const BVH& bvh, const Ray &ray) const{
    return bvh.getIntersection(ray);
}

std::optional<RayCaster::IntersectionData>
RayCaster::findRayIntersection(const Scene& scene, const Ray &ray) const {
    float distance = std::numeric_limits<float>::max();
    RayCaster::IntersectionData result{};
    bool hasIntersection = false;
    auto meshes = scene.getMeshes();
    auto intersections = findRayIntersection(scene.getBVH(), ray);
    for(auto &[meshIndex, triangleIndex]: intersections) {
        ...
        auto &[i0, i1, i2] = indices[triangleIndex];
        if (auto intersection = ray.intersect(vertices[i0], vertices[i1], vertices[i2])) {
            ...
            if (std::fabs(t) < distance) {
                ...
                distance = std::fabs(t);
                result = {meshIndex, triangleIndex, u, v, t};
            }
        }
    }
    //std::cout << "test" << std::endl;
    //std::cout << result.meshIndex << " " << result.triangleIndex << " " << result.u << " " << result.v << " " << result.t << " " << intersections.size() << std::endl;
...
//        }
//    }
    //std::cout << result.meshIndex << " " << result.triangleIndex << " " << result.u << " " << result.v << " " << result.t << std::endl;
    if (!hasIntersection) {
        return std::nullopt;
    } else {
        return {result};
...
RayCaster::sampleCameraPixel(const Scene &scene, const Image &image, size_t i, size_t j, size_t sampleCount) const {
    ...
    for (size_t k = 0; k < sampleCount; k++) {
        ...
        if (auto intersection = findRayIntersection(scene, ray)) {
...
Vec3<float> RayCaster::sampleShadowColor(const Scene &scene, const LightSource &lightSource, const Vec3<float> &hitPos,
    ...
    for (size_t i = 0; i < sampleCount; i++) {
        ...
        if (auto intersectionFromLight = findRayIntersection(scene, ray)) {
